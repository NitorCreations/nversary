...
class AnniversaryService {
   ...
   * @param date
   */
  public getEmployeesToCongratulateToday(
    date: Date,
    maxPerDay: number
  ): CongratulationDay | undefined {
    const congrats = this.getCongratulationsForThisMonth(date, maxPerDay).find(
   ...
   */
  private getCongratulationsForThisMonth(
    date: Date,
    maxPerDay: number
  ): Array<CongratulationDay> {
    ...
    const peopleWithAnniversaryThisMonth = this.employeeRepository
      .findAllEmployees()
      .filter((e) => e.presence[0].start.getMonth() == date.getMonth())
      .filter((e) => e.presence[0].start.getFullYear() !== date.getFullYear())
      .sort(
        (e1, e2) =>
          e1.presence[0].start.getFullYear() -
          e2.presence[0].start.getFullYear()
      );

    const congratulationDaysInThisMonth =
      this.getCongratulationDaysForThisMonth(date);
    peopleWithAnniversaryThisMonth.forEach((employee) => {
      congratulationDaysInThisMonth
        ...
        .sort(
          (d1, d2) =>
            Math.abs(d1.date.getDate() - employee.presence[0].start.getDate()) -
            Math.abs(d2.date.getDate() - employee.presence[0].start.getDate())
        );
  ...
  private getCongratulationDaysForThisMonth(d: Date): Array<CongratulationDay> {
    // TODO add support for vacation days
    const date = new Date();
